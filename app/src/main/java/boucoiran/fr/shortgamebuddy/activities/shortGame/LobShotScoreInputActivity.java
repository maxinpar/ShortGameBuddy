...
public class LobShotScoreInputActivity extends AppCompatActivity {
    ...
    private static String TAG = "LobShotScoreInputActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lob_shot_score_input);
        scDbHelper = new GolfPracticeDBHelper(this);

        setTitle("Lob Shot Drill");
        ...
        mainLayout.setOnTouchListener(new OnSwipeTouchListener(LobShotScoreInputActivity.this) {
            public void onSwipeLeft() {
                iDest = ShortGameScorecardActivity.class;
                saveDrill(null);
            }
    ...
    public void clearLobShotDrill(View view){
        Log.i(TAG,"clearing LobS Drill");

        if(scd.getId() != -1) {
            try {
                //if the drill exists, then delete it.
                scDbHelper.deleteShortGameDrill(scd.getId());
                Log.i(TAG,"LobS Drill deleted successfully");
            }catch (Exception e) {
                Log.e(TAG, "Failed to delete a Lob Shot Drill on clearing");
            }
        }
