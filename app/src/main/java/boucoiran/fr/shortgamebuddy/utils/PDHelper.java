...
public class PDHelper {
    ...
    /*
     * This method will try and return the Putting Drill of type <drillType> attached to
     * card <cardId>
     */

    public static GenericPuttingDrill getDrill(int cardId, int drillType, SQLiteDatabase db) throws Exception {
        GenericPuttingDrill pd;
        String selectQuery = "SELECT  * FROM " + PuttingDrillEntry.TABLE_NAME + " WHERE ("
                + PuttingDrillEntry.COLUMN_DRILL_TYPE + " = " + drillType
                + ") AND (" + PuttingDrillEntry.COLUMN_CARD_ID + " = " + cardId + ")";
    ...
    /*
     * Will return an array of Strings representing the scores for all 6 drills of a Putting Card
     * Will return -1 if drill not completed / started.
     * We build this array by calling getDrillScoreFromCard for each drill type
     */

    public static String[] getDrillScoresFromCard(long cardId, SQLiteDatabase db) {
        String[] toRet = new String[7];
        for (int i = 0; i < 7; i++) {
            try {
                toRet[i] = String.valueOf(getDrillScoreFromCard(cardId, i, db));
    ...
    /*
     * Will return a Putting Drill score based on cardID and Drill Type. Will return -1 if drill not
     * done
     */

    private static long getDrillScoreFromCard(long cardId, int drillType, SQLiteDatabase db) {
        String selectQuery = "SELECT  * FROM " + PuttingDrillEntry.TABLE_NAME + " WHERE ("
                + PuttingDrillEntry.COLUMN_DRILL_TYPE + " = " + drillType
                + ") AND (" + PuttingDrillEntry.COLUMN_CARD_ID + " = " + cardId + ")";
        Cursor c = db.rawQuery(selectQuery, null);
        if (c != null) {
            if (c.getCount() > 0) {
                c.moveToFirst();
                long toRet = c.getLong(c.getColumnIndex(PuttingDrillEntry.COLUMN_TOTAL_SCORE));
                c.close();
                return toRet;
            }
            c.close();
